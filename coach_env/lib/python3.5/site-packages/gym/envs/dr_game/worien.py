...
class WorienEnv(gym.Env):



    def __init__(self):
        if is_ros:
            rospy.init_node('worien_env', anonymous=True)
        ...
        self.static_grid_map = GridMap(free, map_config['resolution'])
        if is_ros:
            self.static_grid_map.show('rviz_global_grid_map')
        ...
        # Get global path
        start_grid_x = int(self.global_mission['start'][0] / self.static_grid_map.resolution + 0.5)
        start_grid_y = int(self.global_mission['start'][1] / self.static_grid_map.resolution + 0.5)
        goal_grid_x = int(self.global_mission['goal'][0] / self.static_grid_map.resolution + 0.5)
        goal_grid_y = int(self.global_mission['goal'][1] / self.static_grid_map.resolution + 0.5)
        start = (start_grid_x, start_grid_y)
        goal = (goal_grid_x, goal_grid_y)
        path = self.global_planner.get_path(start, goal, self.static_grid_map)
        ...
        self.virtual_vehicle = VirtualVehicle(vehicle_state, vehicle_model, 'vehicle_cmd', 'move_vehicle', is_ros=is_ros)

        # Create dwa planner
        self.dwa_planner = DWAPlanner(vehicle_model, '/home/kai/catkin_ws/src/drproj/dwa_planner_config.yaml', self.static_grid_map, is_ros=is_ros)
    ...
    def step(self, action):
        ...
        if is_ros:
            self.virtual_vehicle.show('rviz_virtual_vehicle', self.static_grid_map)
            self.dwa_planner.show_trajectory(best_traj, 'rviz_predicted_trajectory', self.static_grid_map, 'cube')
